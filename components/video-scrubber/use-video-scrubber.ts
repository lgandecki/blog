...
export function useVideoScrubber(config: VideoScrubberConfig, callbacks?: VideoScrubberCallbacks) {
  ...
  const loadSpriteSheet = useCallback(
    async (sheetIndex: number) => {
      ...
      if (
        previewSpritesRef.current.has(sheetIndex) ||
        loadingSheetsRef.current.has(sheetIndex)
      ) {
  ...
  const preloadAdjacentSheets = useCallback(
    (currentSheetIndex: number) => {
      ...
      for (const idx of sheetsToPreload) {
        if (
          idx >= 0 &&
          idx < metadata.numSheets &&
          !previewSpritesRef.current.has(idx)
        ) {
          loadSpriteSheet(idx)
        }
      }
  ...
  const getFrameIndexForTime = useCallback(
    (timeSeconds: number) => {
      ...
      if (times && times.length) {
        ...
        return timeSeconds - times[lo - 1] < times[lo] - timeSeconds
          ? lo - 1
          : lo
      }
  ...
  const computePlayableRegions = useCallback(() => {
    ...
    const buffered = segments.map((seg) => ({
      start: Math.max(0, seg.start - BUFFER),
      end: Math.min(metadata.duration, seg.end + BUFFER),
    }))
  ...
  const renderPreviewFrame = useCallback(
    async (frameIndex: number) => {
      console.log('[renderPreviewFrame] called with frameIndex:', frameIndex)
      ...
      if (!sprite) {
        console.log('[renderPreviewFrame] loading sprite sheet:', pos.sheetIndex)
      ...
      console.log('[renderPreviewFrame] drawing frame:', frameIndex, 'from sheet:', pos.sheetIndex)
      ctx.drawImage(
        sprite,
        pos.preview.x,
        pos.preview.y,
  ...
  const renderTimeline = useCallback(() => {
    ...
    const { thumbnailSize, frameCount, thumbnailCols } = metadata

    const thumbWidth = rect.width / frameCount
    const scale = Math.min(
      thumbWidth / thumbnailSize.width,
      rect.height / thumbnailSize.height
    )
    const displayWidth = thumbnailSize.width * scale
    const displayHeight = thumbnailSize.height * scale
    const y = (rect.height - displayHeight) / 2

    ctx.fillStyle = '#242424'
    ctx.fillRect(0, 0, rect.width, rect.height)

    for (let i = 0; i < frameCount; i++) {
      const thumbCol = i % thumbnailCols
      const thumbRow = Math.floor(i / thumbnailCols)
      const sx = thumbCol * thumbnailSize.width
      const sy = thumbRow * thumbnailSize.height
      const x = (i / frameCount) * rect.width
  ...
  const updatePlayhead = useCallback(() => {
    ...
    if (playheadRef.current && timelineRectRef.current) {
      playheadRef.current.style.left = `${progress * timelineRectRef.current.width}px`
    }
    if (filmstripPlayheadRef.current && filmstripRectRef.current) {
  ...
  const seekToFrame = useCallback(
    (frameIndex: number) => {
      ...
      if (frameIndex === currentFrameRef.current) {
        console.log('[seekToFrame] skipping, same frame:', frameIndex)
        return
      }

      console.log('[seekToFrame] seeking to frame:', frameIndex, 'from:', currentFrameRef.current)

      const times = metadata.frameTimes
      const newTime =
        times && times[frameIndex] != null
          ? times[frameIndex]
          : frameIndex * metadata.interval
  ...
  const play = useCallback(() => {
    ...
    const times = metadata?.frameTimes
    const targetTime =
      times && times[currentFrame] != null ? times[currentFrame] : currentTime
  ...
  useEffect(() => {
    ...
    console.log('[setupCanvases effect] initializing (once)')
    ...
    const setupCanvasLayout = () => {
      ...
      if (timelineCanvas) {
        ...
        if (container) {
          timelineRectRef.current = container.getBoundingClientRect()
          timelineCanvas.width = timelineRectRef.current.width * window.devicePixelRatio
          timelineCanvas.height = timelineRectRef.current.height * window.devicePixelRatio
  ...
  const startScrubbing = useCallback(
    (e: React.MouseEvent) => {
      console.log('[startScrubbing] starting scrub, showVideo:', showVideo)
      wasPlayingBeforeScrubRef.current = isPlaying
      isScrubbingRef.current = true

      // Switch to canvas view for scrubbing
      if (showVideo) {
        console.log('[startScrubbing] hiding video, showing canvas')
        setShowVideo(false)
      }
  ...
  const handleScrub = useCallback(
    (e: React.MouseEvent | MouseEvent) => {
      if (!timelineRectRef.current || !metadata) return

      const x = e.clientX - timelineRectRef.current.left
      const progress = Math.max(0, Math.min(1, x / timelineRectRef.current.width))
      ...
      if (!scrubRafRef.current) {
        scrubRafRef.current = requestAnimationFrame(() => {
          ...
          if (pendingScrubXRef.current !== null && timelineRectRef.current && metadata) {
            const scrubX = pendingScrubXRef.current - timelineRectRef.current.left
            const scrubProgress = Math.max(0, Math.min(1, scrubX / timelineRectRef.current.width))
            const frameIndex = Math.round(scrubProgress * (metadata.frameCount - 1))
            console.log('[handleScrub RAF] frameIndex:', frameIndex, 'currentFrameRef:', currentFrameRef.current)
  ...
  const endScrubbing = useCallback(() => {
    ...
    if (video && metadata) {
      ...
      const targetTime =
        times && times[frame] != null ? times[frame] : currentTime
  ...
  }, [metadata, currentTime, getFrameIndexForTime])
