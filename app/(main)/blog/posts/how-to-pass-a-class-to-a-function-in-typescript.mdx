...
note: When I was migrating this blog post to my personal site, google noticed that we are not passing the arguments to the constructor, and suggested this code instead:

```typescript
function createInstance<A extends User, Args extends any[]>(c: new (...args: Args) => A, ...args: Args): A {
  return new c(...args);
}
```
